# AI Macro v9.6.0 — Universal Data Center Registry

## Authority

`loaders/data_center_registry.py` is the only data-center identity authority in AI Macro.

All domains consume the same canonical `Campus ID` universe. Water, Power, Grid & Storage, Connectivity, Evidence, and the Data Centers page may enrich or aggregate canonical entities; they do not create, split, merge, deduplicate, or rename data-center identities.
...
- canonical campuses;
- canonical facility/building entities;
...
- A source `site_point` remains location evidence. It can attach to an established campus; an uncorroborated point never creates a canonical campus.
...
Every data-center fact has an entity grain. Campus-wide direct measurements take precedence over child rollups. When no campus total exists, facility values roll to the campus; when facility values are unavailable, building values may roll to the campus using the metric's declared aggregation rule.

A campus total and its child measurements are not added together.
...
The v9.6.0 cutover removes the former facility/campus identity modules and former top-level identity-source filenames.

## Geography contract

Macro and Data Centers use the same canonical interactive campus map in `rendering/spatial.py` / `rendering/charts_data_center.py`.

Map points carry canonical `Campus ID` in selection data. A national campus click can drill into the selected state, and state maps preserve campus selection. Water uses the same Campus IDs for its drought/water context and campus selection.
