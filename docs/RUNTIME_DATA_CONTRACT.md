...
7. After the intended refresh/change set is complete, the owner runs the smallest relevant verification for the code that changed and then runs `python helpers/build_release_manifest.py`.
8. The owner reviews the load report and pushes the retained files and matching `data/release_manifest.json` to GitHub.
...
- Developer mode owns the explicit **Generate editorial synthesis** action and zero-cost **Apply last Read** action; the approved automation worker may invoke the same one-call service under hard call ceilings. Public Reader sessions never call OpenAI.
- The Responses API must return the `GeneratedEditorialSynthesis` Pydantic contract. Every prose sentence must identify supporting supplied `fact_id` values.
- A deterministic hard gate blocks unknown, unsupplied, or out-of-domain fact IDs; unsupported numeric claims; invalid incremental membership; omitted required replacements; and invalid Macro scope. Numeric validation includes cited fact labels/context and normalizes formatting-equivalent values such as `300k` and `300,000`; it does not permit rounding or uncited numbers merely because they exist elsewhere. Editorial shape checks remain diagnostics. Nothing rewrites prose or authorizes a retry.
- Every completed API response is persisted before validation under `openai_artifacts/attempts/<attempt_id>.json`, together with its exact raw response, evidence snapshot, structured model output when available, generation metadata, prompt versions, and eventual validation result.
- `openai_artifacts/evaluated.json` is atomically advanced after every completed response, including explicit retention and hard rejection. It records the evidence baseline, bounded fact history, analytical state, decision, and validation outcome. A transport failure with no response does not advance it.
- Only a hard-gate-passing `publish` response is promoted atomically to `openai_artifacts/current.json`. The artifact records its evidence snapshot, model, reasoning effort, prompt provenance, diagnostics, exact response metadata, Reader payload, and publication freshness lease. Neither attempt, evaluated, nor current artifacts contain an API credential.
- Reader mode publishes the last good artifact as `validated` or `published_with_warnings`. A later deterministic refresh may make `evidence_current=false` without removing the retained Read. Exact or immaterial evidence can renew it without an API call; `retain_prior` preserves it after a completed evaluation. A rejected response leaves it unchanged and is not retried automatically for the same evaluated snapshot.
...
- Paid OpenAI artifacts are runtime/publication state, not retained-data source-release inputs. `helpers/build_release_manifest.py` deliberately excludes `openai_artifacts/`. `openai_artifacts/attempts/` stays local/private; `openai_artifacts/current.json` and `openai_artifacts/evaluated.json` may be intentionally committed for hosted Reader publication/evaluation continuity until another storage path replaces Git publication.
...
- The Census BTOS AI supplement is retained at `data/adoption_ai_supplement_2026.csv` as a dated structural benchmark. Public Reader processes never download it. An authorized Adoption refresh may validate and replace the normalized retained table from the fixed official workbook; a missing or invalid retained table is an explicit unavailable state, not an empty successful load.
