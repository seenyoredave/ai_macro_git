"""Refresh local water context for canonical data-center campuses."""
...
def _write_manifest(*, usdm_report: dict, pws_report: dict) -> pd.DataFrame:
    ...
    values={"source_name":"Public Water System Service Areas, Version 3","custodian":"U.S. Environmental Protection Agency","canonical_url":"https://www.epa.gov/ground-water-and-drinking-water/public-water-system-service-areas","acquisition_url":"https://services.arcgis.com/cJ9YHowT8TU7DUyn/ArcGIS/rest/services/Water_System_Boundaries/FeatureServer/0","publication_date":"2026-03-17","coverage_period":"Version 3; service data updated through 2025/2026 source vintages","geographic_coverage":"Community-water-system service-area intersections for canonical campus points","data_role":"Public-water service-area geography and boundary provenance","evidence_grade":"A/B","resilience_grade":"R1","evidence_class":"agency_estimate","source_kind":"ArcGIS REST point-intersection query cache","license":"U.S. government public data","raw_retention_allowed":"yes","parser_version":"epa-pws-campus-v3.2","schema_version":WATER_LEDGER_VERSION,"refresh_frequency":"periodic","ingestion_status":"active" if pws_active else "identified_not_ingested","source_health":"retained_query_cache" if pws_active else "identified","notes":"EPA community-water-system service-area boundaries are joined to canonical Campus IDs by campus point."}
...
def refresh_local_water_context(campuses: pd.DataFrame | None, *, pws_max_workers: int = 6) -> dict:
    ...
    if not frame.empty and ("Campus ID" not in frame.columns or frame["Campus ID"].duplicated().any()):
        raise ValueError("Local water refresh requires one row per canonical Campus ID")
