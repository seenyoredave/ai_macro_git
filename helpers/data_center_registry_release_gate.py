#!/usr/bin/env python3
"""Full retained-data release gate for AI Macro v9.6.2 data-center identity.
...
def _quality_gate(payload: dict) -> None:
    ...
    if campuses.empty or "Campus ID" not in campuses.columns:
        raise AssertionError("Campus table is empty")
    if not campuses["Campus ID"].is_unique:
        raise AssertionError("Campus IDs are duplicated")
    if "Campus Label" not in campuses.columns or not campuses["Campus Label"].astype(str).is_unique:
        raise AssertionError("Campus labels are missing or duplicated")
    ...
    if labels.eq("").any() or names.eq("").any():
        raise AssertionError("Campus contains a blank name/label")
    if labels.str.contains(" · ", regex=False).any():
        raise AssertionError("Legacy concatenated selector labels remain in the registry")
    ...
    if generated_numbering.any():
        ...
        raise AssertionError(
            f"Ungrounded dash-number identity labels remain in the registry: {bad.head(10).to_dict('records')}"
        )
    ...
    if labels.str.contains("Unidentified data-center site", case=False, regex=False).any():
        raise AssertionError("Unidentified placeholder campuses remain in the registry")
    ...
    if missing_state_in_label:
        raise AssertionError(
            f"Campus labels do not expose their U.S. state jurisdiction: {missing_state_in_label[:10]}"
        )
    ...
    registry_ids = _ids(campuses)
    if set(entities.get("Campus ID", pd.Series(dtype=str)).dropna().astype(str)) - {""} - registry_ids:
        raise AssertionError("Entity hierarchy contains unknown Campus IDs")
    if set(membership.get("Campus ID", pd.Series(dtype=str)).dropna().astype(str)) - {""} - registry_ids:
    ...
    if "Anchor Observation ID" in campuses.columns:
        ...
        if point_anchors:
            raise AssertionError(f"Point observations anchor campuses: {point_anchors[:10]}")
...
def _umatilla_gate(payload: dict) -> None:
    ...
    if len(resolved) != 4:
        raise AssertionError("Umatilla membership references a missing campus")
    if resolved["Campus Label"].astype(str).str.match(r"^Umatilla County(?:\s|$)", case=False).any():
        raise AssertionError("A county-only Umatilla placeholder became a campus")
    ...
    for group, rows in decisions.groupby("Decision Group", sort=False):
        ...
        if decision == "merge":
            ...
            if len(set(flattened)) != len(flattened):
                failures.append(f"{group}: reviewed separation shares a campus")
        else:
...
def _domain_gate(payload: dict) -> None:
    ...
    if _ids(water_campuses) != registry_ids:
        raise AssertionError("Water does not preserve the full campus universe")
...
def main() -> int:
    ...
    print(
        "PASS  v9.6.2 retained national release gate · "
        ...
        "Data Centers/Water/Connectivity share Campus IDs"
    )
    return 0
