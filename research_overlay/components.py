"""Reusable Streamlit components for the research overlay."""
...
from research_overlay.visuals import compact_sparkline
...
def render_section(title: str, subtitle: str | None = None, *, first=False) -> None:
    ...
    st.markdown(
        f"""
        <div class="rm-section{first_class}">
...
def metric_card(
    *,
    ...
    source_text = str(source or "Current")
...
def render_statline(
    ...
    *,
    ...
    """Render compact summary cards under an explicit Streamlit key namespace.

    Reusable render helpers can be invoked more than once from the same source
    line, so call-site inference is not sufficient for Streamlit element keys.
    Every caller supplies a stable semantic namespace instead.
    """
    items = list(stats)
    if not items:
        return
    ...
    for index, (column, item) in enumerate(zip(columns, items)):
        ...
        st.markdown(
            ...
            .st-key-{key} {{
                border: 1px solid rgba(148, 163, 184, 0.17) !important;
                border-top: 3px solid rgba(96, 165, 250, 0.85) !important;
                border-radius: 14px !important;
                background: rgba(17, 24, 39, 0.82) !important;
                padding: 0.2rem 0.85rem 0.45rem 0.85rem !important;
                min-height: 128px;
            }}
            </style>
            """,
...
def render_static_table(frame: pd.DataFrame | None, *, empty_message: str = "No rows available.") -> None:
    """Render a small research table as HTML, bypassing Arrow entirely."""
