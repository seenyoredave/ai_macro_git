...
def normalize_borrower_strain_history(history):
    ...
    out = history.copy()
    out["Borrower Strain"] = pd.to_numeric(out["Borrower Strain"], errors="coerce")
    if "Borrower Strain Version" in out.columns:
        out["Borrower Strain Version"] = out["Borrower Strain Version"].astype("string")
    else:
...
def _normalize_commitment_ledger(df, *, as_of_date=None) -> pd.DataFrame:
    ...
    if as_of_date is not None:
        ...
        if pd.isna(cutoff):
            raise ValueError(f"Invalid borrower-strain as_of_date: {as_of_date}")
...
def calculate_borrower_strain(
    ...
    *,
    ...
    signed_scores = {
        name: borrower_strain_to_signed(score)
        for name, score in base_scores.items()
    }
    signed_score = borrower_strain_to_signed(combined["score"])
    ...
    components = {
        "Cash Flow Strain": {
            ...
            "weight": BORROWER_STRAIN_WEIGHTS["Cash Flow Strain"],
        ...
        "Debt Capacity Strain": {
            ...
            "weight": BORROWER_STRAIN_WEIGHTS["Debt Capacity Strain"],
        ...
        "Committed Burden": {
            ...
            "weight": BORROWER_STRAIN_WEIGHTS["Committed Burden"],
        ...
        "Contingent Exposure": {
            ...
            "weight": BORROWER_STRAIN_WEIGHTS["Contingent Exposure"],
    ...
    return {
        ...
        "target_cohort_size": len(BORROWER_STRAIN_TICKERS),
