"""Deterministic local water-constraint evidence summaries.

The functions in this module describe observability and physical exposure. They
never estimate data-center water use and never treat service-area overlap as a
confirmed customer relationship.
"""
...
def _capacity(frame: pd.DataFrame) -> pd.Series:
    ...
    for column in candidates:
        if column in frame.columns:
            candidate = _number(frame[column])
            output = output.where(output.notna(), candidate)
    return output.where(output > 0)


def local_water_constraint_summary(facility_context: pd.DataFrame | None) -> dict:
    frame = facility_context.copy() if isinstance(facility_context, pd.DataFrame) else pd.DataFrame()
    if frame.empty:
        return {
            "mapped_facilities": 0,
            "county_drought_resolved": 0,
    ...
    local_overlap_d2 = pws_overlap & any_d2
    local_authoritative_d2 = pws_authoritative & any_d2

    highest = frame.loc[county_d2.idxmax()] if county_d2.notna().any() else pd.Series(dtype=object)
    highest_d2 = float(county_d2.max()) if county_d2.notna().any() else np.nan
    highest_location = ", ".join(
        part for part in [str(highest.get("County") or "").strip(), str(highest.get("State") or "").strip()] if part
    )
    ...
    return {
        "mapped_facilities": int(len(frame)),
        "county_drought_resolved": int(county_d2.notna().sum()),
        ...
        "service_area_provenance_classified_share": (
            float(pws_classified.sum() / pws_overlap.sum()) if int(pws_overlap.sum()) > 0 else np.nan
        ),
        ...
        "published_capacity_records": capacity_records,
        "published_capacity_coverage_share": float(capacity.notna().mean()),
        "facilities_in_counties_with_d2": int(any_d2.sum()),
        "facilities_in_counties_with_d2_share": float(any_d2.loc[county_d2.notna()].mean()) if county_d2.notna().any() else np.nan,
        "facilities_in_counties_with_25pct_d2": int(material_d2.sum()),
        "facilities_in_counties_with_25pct_d2_share": float(material_d2.loc[county_d2.notna()].mean()) if county_d2.notna().any() else np.nan,
        ...
        "service_area_overlap_in_counties_with_d2": int(local_overlap_d2.sum()),
        "authoritative_overlap_in_counties_with_d2": int(local_authoritative_d2.sum()),
