...
def infrastructure_attribution(history: pd.DataFrame | None) -> dict:
    ...
    specs = {
        ...
        "Electric power": ("Electric Power Construction", "Private Nonresidential Construction"),
        "Communications": ("Communication Construction", "Private Nonresidential Construction"),
    }
    expected_columns = []
    excess_columns = []
    component_rows = []
    for label, (observed_column, denominator_column) in specs.items():
        ...
        frame[excess_column] = (frame[observed_column] - frame[expected_column]).clip(lower=0)
        expected_columns.append(expected_column)
        excess_columns.append(excess_column)
        valid = frame[["Observation Date", observed_column, expected_column, excess_column]].dropna().tail(1)
        if not valid.empty:
            row = valid.iloc[0]
    ...
    observed_columns = [observed for observed, _ in specs.values()]
    output = pd.DataFrame({
        "Date": frame["Observation Date"],
        "Direct AI Construction": frame["Data Center Construction"],
        "Broader Supporting Construction": frame[observed_columns].sum(axis=1, min_count=len(observed_columns)),
        "Expected Baseline": frame[expected_columns].sum(axis=1, min_count=len(expected_columns)),
