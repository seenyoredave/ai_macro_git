...
17. A completed editorial response may replace the last good Read only after a minimal hard publication gate confirms required structural fields and factual grounding: supplied support IDs and supported displayed numbers. Style, cadence, punctuation, sentence-count, numerical-density, and similar editorial preferences are diagnostics by default, not publication blockers. Rejection retains the last good Read; bots must not rewrite or repair rejected prose locally.
18. Routine finding changes and newly qualified developments create candidates for editorial refresh; they do not make the last good Read unpublishable or force every candidate domain to be rewritten. The model may leave a domain Read unchanged when it has nothing materially better to say. A rejected response does not advance the editorial comparison baseline or suppress a later authorized attempt. The prior good Read remains published until a grounded replacement passes the minimal gate.
