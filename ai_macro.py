...
from rendering.snapshot_status import retained_market_snapshot_label
...
APP_VERSION = "v6.9.1"
APP_STATE_SCHEMA_VERSION = "64.0-v6.9.1-retained-loader-policy"
...
def _record_domain_refresh(domain: str, report: dict | None) -> None:
    ...
    st.session_state.last_domain_refresh = {
        "domain": domain,
        "label": DOMAIN_REFRESH_LABELS.get(domain, domain),
        "source_mode": mode,
...
def render_developer_load_report(report):
    ...
    policy = dict(st.session_state.get("current_load_policy", {}) or {})
    if policy:
        sources = policy.get("refresh_sources") or []
        source_text = ", ".join(sources) if sources else "none"
        st.caption(
            f"Load policy: {policy.get('mode', 'unknown')} · "
            f"authorized live sources: {source_text}"
        )
    ...
    if write_report:
        ...
        st.markdown("**Snapshot writes**")
        st.write(f"Status: `{write_report.get('status', 'unknown')}`")
        st.write(f"Written: `{', '.join(written) if written else 'none'}`")
...
if developer_mode():
    with st.sidebar:
        ...
        st.caption(
            "Rebuild and cache clearing stay offline. Only the named provider "
            "and domain controls below may contact upstream sources."
        )
        ...
        st.markdown("---")
        st.markdown("**Refresh financials**")
...
if st.session_state.force_rebuild:
    refresh_domain = st.session_state.get("domain_refresh_request")
    domain_tokens = dict(st.session_state.get("domain_refresh_tokens", {}) or {})
    domain_token = int(domain_tokens.get(refresh_domain, 0) or 0) if refresh_domain else 0
    ...
    energy_market_scope = (
        "power" if refresh_domain == "power"
        else "grid_storage" if refresh_domain == "grid_storage"
        else "all"
    )
    energy_data = load_energy_data(
        fred_data=fred_data,
        force_refresh=refresh_domain == "power",
        refresh_token=domain_token if refresh_domain == "power" else 0,
        force_fred_refresh=st.session_state.force_fred_refresh,
        fred_refresh_token=st.session_state.fred_refresh_token,
        force_market_refresh=refresh_domain in {"power", "grid_storage"},
        market_refresh_token=domain_token if refresh_domain in {"power", "grid_storage"} else 0,
    ...
    infrastructure_data = load_infrastructure_data(
        refresh_token=domain_token,
        force_construction_refresh=refresh_domain in {"data_centers", "grid_storage"},
        force_facility_refresh=refresh_domain == "data_centers",
        force_compute_refresh=refresh_domain == "compute",
    )
    connectivity_data = load_connectivity_data(
        infrastructure_data.get("campus_registry"),
        force_refresh=refresh_domain == "connectivity",
        refresh_token=domain_token if refresh_domain == "connectivity" else 0,
    )
    ...
    construction_data = load_data_center_construction(
        force_refresh=False,
        refresh_token=domain_token if refresh_domain in {"data_centers", "grid_storage"} else 0,
        allow_live=False,
    )
    ...
    water_data = load_water_utilization_data(
        force_refresh=refresh_domain == "water",
        refresh_token=domain_token if refresh_domain == "water" else 0,
    )
    infrastructure_data, water_data = attach_water_context(infrastructure_data, water_data)
    adaptation_data = load_adaptation_data(
        force_refresh=refresh_domain == "adoption",
        refresh_token=domain_token if refresh_domain == "adoption" else 0,
        allow_live=load_policy.allows_live(RefreshSource.ADOPTION),
    )
    workforce_data = load_workforce_data(
        force_refresh=refresh_domain == "workforce",
        refresh_token=domain_token if refresh_domain == "workforce" else 0,
    )
    economic_impact_data = load_economic_impact_data(
        force_refresh=refresh_domain == "economic_outcomes",
        refresh_token=domain_token if refresh_domain == "economic_outcomes" else 0,
    )
    commercialization_refresh = refresh_domain in {
        "compute",
        "adoption",
        "economic_outcomes",
    }
    commercialization_data = load_commercialization_data(
        ...
        refresh_token=domain_token if commercialization_refresh else 0,
    )
    ...
    if refresh_domain:
        ...
        _record_domain_refresh(refresh_domain, domain_report)
...
render_masthead(
    "AI Macro",
    "An economic research platform that traces the AI economy from capital and construction through deployment, adoption, and economic results.",
    version=APP_VERSION,
    status=retained_market_snapshot_label(),
)
