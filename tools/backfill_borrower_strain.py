#!/usr/bin/env python3
"""Build the retained Borrower Strain history from public SEC filings.
...
5. calculates Borrower Strain with the current engine; and
...
The command resumes from ``.cache/borrower_strain_backfill``.  Set
...
from analytics.borrower_strain_history import (
    ...
    borrower_strain_result_to_history_row,
    historical_observation_dates,
    snapshot_to_sector_data,
)


CACHE_DIR = PROJECT_ROOT / ".cache" / "borrower_strain_backfill"
...
def build_history(
    ...
    for cutoff, frequency in observations:
        ...
        result = calculate_borrower_strain(
            sector_data,
            as_of_date=cutoff,
...
def main():
    ...
    if args.reuse_raw:
        ...
        if fundamentals.empty:
            ...
            if fundamentals.empty:
                raise RuntimeError(
                    "Borrower Strain fundamentals could not be restored from the local SEC cache. "
    ...
    accepted = int(history["Borrower Strain"].notna().sum()) if not history.empty else 0
    print("\nBorrower Strain backfill complete")
