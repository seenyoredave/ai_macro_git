...
from analytics.capital_stress_history import (
    ...
    combine_capital_stress_history,
    historical_observation_dates,
    ttm_fact,
)
from tools.backfill_capital_stress import Filing, accepted_ledger, extract_commitment_candidates
...
def test_combine_history_prefers_live_row_on_duplicate_date():
    ...
    result = combine_capital_stress_history(live, backfill)
    assert len(result) == 1
    assert result.iloc[0]["Capital Stress"] == 12.0


def test_backfill_history_constitutes_with_three_components():
    from tools.backfill_capital_stress import build_history
    ...
    assert pd.notna(history.iloc[0]["Capital Stress"])
