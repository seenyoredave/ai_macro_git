from __future__ import annotations

from analytics.macro_dataframe import build_macro_dashboard_data
from rendering.adaptation import render_adaptation_tab
from rendering.compute import render_compute_tab
from rendering.data_center import render_data_center_tab
from rendering.economic_impact import render_economic_impact_tab
from rendering.evidence import render_evidence_tab
from rendering.finance import render_finance_tab
from rendering.grid_storage import render_grid_storage_tab
from rendering.macro import render_macro_tab
from rendering.market import render_market_tab
from rendering.power import render_power_tab
from rendering.water import render_water_tab
from rendering.workforce import render_workforce_tab


def render_research_dashboard(
    tabs,
    sector_data,
    sector_metrics,
    fred_data,
    regime_metrics,
    nfci_history=None,
    energy_data=None,
    debt_markets_data=None,
    infrastructure_data=None,
    water_data=None,
    adaptation_data=None,
    workforce_data=None,
    economic_impact_data=None,
    market_universe_summary=None,
    sector_weekly_context=None,
    dashboard_data=None,
    platform_reads=None,
):
    dashboard_data = dashboard_data or build_macro_dashboard_data(
        sector_metrics=sector_metrics,
        regime_metrics=regime_metrics,
    )
    platform_reads = platform_reads or {}

    with tabs[0]:
        render_macro_tab(
            sector_metrics,
            sector_data,
            fred_data,
            regime_metrics,
            dashboard_data,
            adaptation_data or {},
            infrastructure_data or {},
            tab_read=platform_reads.get("macro"),
        )
    with tabs[1]:
        render_market_tab(
            sector_metrics,
            sector_data,
            regime_metrics,
            dashboard_data,
            market_universe_summary,
            weekly_context=sector_weekly_context,
            tab_read=platform_reads.get("market"),
        )
    with tabs[2]:
        render_finance_tab(
            sector_metrics,
            sector_data,
            fred_data,
            regime_metrics,
            nfci_history,
            debt_markets_data or {},
            dashboard_data,
            tab_read=platform_reads.get("finance"),
        )
    with tabs[3]:
        render_compute_tab(infrastructure_data or {}, tab_read=platform_reads.get("compute"))
    with tabs[4]:
        render_data_center_tab(infrastructure_data or {}, tab_read=platform_reads.get("data_center"))
    with tabs[5]:
        render_power_tab(
            fred_data,
            regime_metrics,
            energy_data or {},
            dashboard_data,
            infrastructure_data or {},
            tab_read=platform_reads.get("power"),
        )
    with tabs[6]:
        render_grid_storage_tab(
            energy_data or {},
            infrastructure_data or {},
            tab_read=platform_reads.get("grid_storage"),
        )
    with tabs[7]:
        render_water_tab(water_data or {}, infrastructure_data or {}, tab_read=platform_reads.get("water"))
    with tabs[8]:
        render_adaptation_tab(adaptation_data or {}, tab_read=platform_reads.get("adaptation"))
    with tabs[9]:
        render_workforce_tab(workforce_data or {}, tab_read=platform_reads.get("workforce"))
    with tabs[10]:
        render_economic_impact_tab(economic_impact_data or {}, tab_read=platform_reads.get("economic_impact"))
    with tabs[11]:
        render_evidence_tab(
            fred_data,
            sector_data,
            sector_metrics,
            regime_metrics,
            energy_data or {},
            debt_markets_data or {},
            dashboard_data,
            infrastructure_data or {},
            water_data or {},
            adaptation_data or {},
            workforce_data or {},
            economic_impact_data or {},
        )
